defmodule Hatoba.Queue do
  ...
  def add(arg) when is_list(arg) do
    res = Enum.map(arg, fn(a) ->
      GenServer.call(:download_queue, {:add, a})
    end)

    if Enum.any?(res, fn(a) -> a != :ok end) do
  ...
  def handle_call({:status}, _from, state) do
    ...
    |> Enum.map(fn(x) -> Enum.map(x, &(Hatoba.Download.status(&1))) end)
  ...
  defp do_add(arg, state, dest \\ %Hatoba.UploadType{}) do
    ...
    {new_dl, new_q} = if Kernel.map_size(downloading) >= @max_queue_size do
