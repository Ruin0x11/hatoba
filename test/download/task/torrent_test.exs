defmodule Hatoba.Download.TorrentTest do
  ...
  def run(url) do
    Hatoba.Download.StdoutTask.async([Hatoba.Download.Torrent, self(), "/tmp", url])
  end

  def run_unlinked(url) do
  ...
  test "provides progress" do
    with_mock Porcelain, [spawn_shell: fn(_, _) -> %{ :pid => 0 } end] do
      %Task{pid: pid} = run("https://nyaa.si/download/1.torrent")
  ...
  test "reports success" do
    with_mock Porcelain, [spawn_shell: fn(_, _) -> %{ :pid => 0 } end] do
      task = %Task{pid: pid} = run("https://nyaa.si/download/1.torrent")
  ...
  test "reports failure" do
    with_mock Porcelain, [spawn_shell: fn(_, _) -> %{ :pid => 0 } end] do
      {:ok, pid} = run_unlinked("https://nyaa.si/download/1.torrent")
